...
def register_result(request):
	...
	if request.method == 'POST':
		print 'Inside register_result and if loop'
		firstname = request.POST.get('firstname')
		lastname = request.POST.get('lastname')
...
# year and department are 'blank' other are ''

		if firstname == '' or lastname == '':
			context['error_name'] = '*Firstname or lastname cannot be empty'
		else:
			firstname, lastname = validate_name(firstname,lastname)
...
def validate_name(firstname,lastname):
	...
	firstname = re.match('^([a-zA-Z]| |.)+$',firstname)
...
def validate_email(email):
	...
	temp = re.match('^(([a-z]+)|([A-Z]+)).[a-z]{2,3}[0-9]{2}@iitp.ac.in$',email)
	if temp is None :
		return None
	else : 
		return temp.group()
